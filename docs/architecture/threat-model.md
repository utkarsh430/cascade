...
| T3 | **Prompt injection through the corpus.** Evidence is scraped news text placed in the agent's prompt; a document could carry instructions | Partial. Agents act only through one schema-constrained tool; an action the actor cannot take is coerced to WAIT and recorded (ADR-0018); the arbiter that folds actions into the world is deterministic Python with no model call (invariant 3). An injected instruction can bias an agent's *choice among legal actions*; it cannot make it take an illegal one or touch the arbiter | **residual risk — probe built, not yet run.** `cascade eval injection` (M14) forecasts a keyed-hash sample on its retrieved evidence and again with one appended document that asserts nothing about the world and instructs the model to report a probability contradicting its clean answer — three attack forms, compliance and movement with Wilson intervals, a broken output format counted rather than scored as unmoved. It measures the single-model forecaster, whose evidence block is formatted as the agents' is; it needs the corpus and a model, and runs after compile |
