...
| [0046](0046-publishing-and-recovery-drills.md) | A published report carries the labels, so it lands in a private, versioned, Object-Locked bucket the study task cannot read back, and a CloudFront site is rejected; a restore role that provably cannot write its own source; the tier-0 archive keyed by the seal it belongs to, with a scheduled inventory instead of a scheduled upload | M12 |
